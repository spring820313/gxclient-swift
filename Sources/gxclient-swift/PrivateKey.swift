...
public struct PrivateKey: Equatable {
    ...
    public func sharedSecret(pubkey: PublicKey) -> Data? {
        guard let myPubKeyBytes = Secp256k1Context.shared.getUncompressedPublicKey(fromBytes: pubkey.key) else {
            return nil
        }
        ...
        let sharedSecretComputeStatus = uECC_shared_secret([UInt8](myPubKeyBytes.suffix(from: 1)), [UInt8](self.secret), sharedSecret)
        
        guard sharedSecretComputeStatus == 1 else {
            
            fatalError("Couldn't compute shared secret")
        }
        return Data(UnsafeBufferPointer(start: sharedSecret, count: 32))
    }
